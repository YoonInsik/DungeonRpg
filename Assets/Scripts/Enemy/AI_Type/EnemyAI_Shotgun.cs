...
namespace SHS
{
    public class EnemyAI_Shotgun : MonoBehaviour
    {
        ...
        IEnumerator Sniping()
        {
            ...
            EnemyAttackBullet[] bullets = C.GetComponentsInChildren<EnemyAttackBullet>();

            for(int i = 0; i < bullets.Length; i++)
            {
                bullets[i].damamge = m_enemy.m_stat.damage + m_enemy.m_stat.damage_scaling * (EnemySpawner_v3.Instance.Get_WaveLevel() - 1);
            }
